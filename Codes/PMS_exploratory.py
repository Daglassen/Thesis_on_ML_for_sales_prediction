...
### 1. SPLIT AND SHUFFLE DATA
test_proportion = 0.25;
data_train, data_test = train_test_split(df,test_size=test_proportion, random_state=0, shuffle=True);
X_train = data_train.drop('done', axis=1)
X_test = data_test.drop('done', axis=1)
y_train = data_train['done']
y_test = data_test['done']
...
### 3. STANDARDIZE TEST/TRAIN SETS
X_train['Running_Hours'] = MinMaxScaler().fit_transform(np.array(X_train['Running_Hours']).reshape(-1,1))
X_test['Running_Hours'] = MinMaxScaler().fit_transform(np.array(X_test['Running_Hours']).reshape(-1,1))


### 4. EMBEDDING TEST/TRAIN SETS
'''
...
ScoreRandForest = metrics.accuracy_score(y_test, y_pred_RF)
cmRandForest = metrics.confusion_matrix(y_test, y_pred_RF)
...
df1['Running_Hours'] = MinMaxScaler().fit_transform(np.array(df1['Running_Hours']).reshape(-1,1))
#set input/output
Y = df1['done']
X = df1.drop(['done'], axis=1)
...
model.predict(X) # prob of successes (done)
np.round(model.predict(X),0) # 1 and 0 (done or not)
Y # 1 and 0 (done or not)
