...
### 5. PCA
pca = PCA(n_components = 59)  # the number is equal to the number of columns (we do it to plot the explained variance)
pca.fit(X_train)
...
# take components that explain 97% of the variance
...
### 1. LINEAR REGRESSION
LogReg.fit(X_train_pca, y_train)  # fit on the transformed set
y_pred_log = LogReg.predict(X_test_pca)

ScoreLogReg = LogReg.score(X_test_pca,y_test)
...
clf.fit(X_train_pca, y_train)
y_pred_GBC = clf.predict(X_test_pca)
...
model.predict(X_test_pca) # prob of successes (done)
np.round(model.predict(X_test_pca),0) # 1 and 0 (done or not)


# Round predictions to a whole number (0 or 1)
preds = np.round(model.predict(X_test_pca),0)

# confusion matrix
cmNN = metrics.confusion_matrix(y_test, preds) 

# classification report
metricsNN = metrics.classification_report(y_test, preds)
